...
class BlackjackGameGym(gym.Env):
    def __init__(self):
        super().__init__()
        ...
        self.observation_space = Box(
            low=np.array([2, 4, -10, 0]),
            high=np.array([11, 20, 10, 1]),
            dtype=np.int32
        )
        self.action_space = Discrete(2)  # 0 = stand, 1 = hit

        # Initialize state and reset
        self.state = None
        self.initialize()
    

    def initialize(self, true_count=0):
        """
        Initialize the game state. If a specific true count is provided, reset the deck and state
        ...
        if true_count == 0:
            ...
            self.state = [0, 0, self.cards.true_count, 0]
    ...
    def deal_initial_hands(self):
        ...
        self.state[3] = int(self.player_hand.usable_aces)
    ...
    def step(self, action):
        ...
        if action == 1:  # Hit
            ...
            raise ValueError("Invalid action. Action must be 0 (stand) or 1 (hit).")

        # No truncation logic in this environment
        truncated = False
    ...
    def dealer_play(self):
        """
        Logic for the dealer's actions during their turn.
        """
        while self.dealer_hand.hand_value < 17 or (self.dealer_hand.hand_value == 17 and self.dealer_hand.soft):
            self.dealer_hand.add_card(self.cards.draw_card())
    ...
    def player_hit(self):
        ...
        usable_aces = int(self.player_hand.usable_aces)
